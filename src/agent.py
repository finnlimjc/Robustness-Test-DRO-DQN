...
class DQNBase(AgentInterface):
    ...
    def __init__(self, state_dim:int, action_dim:int, action_values:np.ndarray, total_paths:int, batch_size:int, n_updates:int,
        ...
        # Loss Functions
        self.network_optimizer = (torch.optim.Adam(self.q.parameters(), lr=network_lr) 
                                  if network_optimizer is None else network_optimizer)
...
class RobustDQNBase(DQNBase):
    ...
    def log_indicators(self, rewards:torch.Tensor, next_states:torch.Tensor, not_terminal:torch.Tensor,
                       targets:torch.Tensor, lambda_iters:int, lambdas:torch.Tensor, mask:torch.Tensor):
        """
...
class PORDQN(RobustDQNBase):
    ...
    def prepare_target_states(self, state:torch.Tensor, action:torch.Tensor, next_return_from_prior:torch.Tensor,
                              sampled_states:torch.Tensor, realized_return:torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """
...
class EmpiricalPortfolioDRODQN(PORDQN):
    """
    Extends PORDQN to optimize a single shared lambda over the batch empirical
    distribution, rather than one lambda per buffer sample. Treats the batch as draws from a
    stationary mixture of market regimes (assuming a well-calibrated replay buffer).
    
    The scalar self.lambda_val is updated each training step and serves as the warm start for the
    next call, replacing the per-sample lambda caching scheme in the replay buffer.
    ...
    def _cache_lambdas(self, lamdas:torch.Tensor, buffer_idx:torch.Tensor, mask:torch.Tensor):
        self.lambda_val = lamdas # scalar warm start for next training step
    ...
    def train_batch(self, states:torch.Tensor, actions:torch.Tensor, rewards:torch.Tensor, next_state:torch.Tensor,
        ...
        with torch.no_grad():
            ...
            target_state, reference_return = self.prepare_target_states(
                states, action_weight, next_return_from_prior, next_state, realized_return=rewards_from_prior)
            q_targets = self.target_q(target_state)
            optimal_q_targets = q_targets.max(dim=-1).values
        ...
        hq_value, lamda_star, n_iter = hq_opt_empirical(
            self.duality_operator, reference_return, next_return_from_prior, rewards_from_prior,
        ...
        if self.writer is not None:
            lambdas_expanded = lamda_star.unsqueeze(0).expand(self.batch_size)
            self.log_indicators(rewards, next_state, not_terminal, hq_value, n_iter, lambdas_expanded, mask)
            self.writer.log_corrected_radius(epsilon_bar, self.q_updates)
            self.writer.log_policy_action(action_weight, self.q_updates, decimal_places=1)
