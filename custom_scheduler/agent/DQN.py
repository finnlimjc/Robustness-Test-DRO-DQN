...
class PORDQN:
    '''
    Collapsed Portfolio Optimization Robust DQN class
    All methods from DQN, RDQN, and PORDQN combined into a single class
    
    Uses a student-t distribution for sampling distribution nu
    Tailored for the MMD simulator environment
    '''
    def __init__(
        self,
        obs_dim: int,
        ...
        lamda_lr: float = 0.02,
        lamda_max_iter: int = 100,
        lamda_step_size: int = 10,
        lamda_gamma: float = 10,
        norm_ord: int = 1,
    ...
    def agent_step(self, reward, observation, info=None):
        ...
        if self.training_mode: 
            self.train_mode_actions(reward, observation, False, info)  # NOTE: must be before new action/obs replaces self.prev_action/self.prev_state
        self.prev_state = observation
        self.prev_action = action
        return action
    ...
    def agent_end(self, reward, observation, info=None):
        ...
        reward = self.check_reward(reward)
        if self.training_mode: 
            self.train_mode_actions(reward, observation, True, info)
    ...
    def train_mode_actions(self, reward, observation, terminal, info=None):
        ...
        if self.clone_q_net_condition(): 
            self.clone_q()
        if self.training_condition(): 
            self.update_q()
    ...
    def train_batch(self, current_states, actions, rewards, next_states, terminal_state, lamda_inits, info, buffer_indices):
        ...
        targets, lambdas, lambda_iters, mask = hq_opt(
            self.target_q, 
            current_states, 
            act_values, 
