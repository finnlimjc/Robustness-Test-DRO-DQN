...
class GenLSTM(nn.Module):
    ...
    def __init__(self, noise_dim, seq_dim, seq_len, hidden_size=64, n_lstm_layers=1, activation='Tanh'):
        ...
        self.rnn = nn.LSTM(input_size=seq_dim+noise_dim+1, hidden_size=hidden_size, num_layers=n_lstm_layers, batch_first=True, bidirectional=False, device=self.device)
        self.output_net = nn.Linear(hidden_size, seq_dim, device=self.device)
    ...
    def _generate_sequence(self, last_return, noise, dts, h, c):
        ...
        if noise.ndim == 4:
            ...
            # print(f'last_return.shape: {last_return.device}, noise.shape: {noise.device}, dts.shape: {dts.device}, h.shape: {h.device}, c.shape: {c.device}')
        else:
            n_outer = None
...
class MMDSimulator(gym.Env):
    ...
    def __init__(self,
        ...
        self.bias = torch.tensor(ma_model_params['omega'], dtype=DATATYPE, device=self.device)
        self.lags = torch.tensor(ma_model_params.values[1:], dtype=DATATYPE, device=self.device).flip(0).unsqueeze(-1) # flip to match the order of seq
    ...
    def reset(self, seed: Optional[int]=None):
        ...
        self.position = torch.zeros((self.batch_size, self.seq_dim), dtype=DATATYPE, requires_grad=False, device=self.device)
        self.log_wealth = torch.zeros((self.batch_size, 1), dtype=DATATYPE, requires_grad=False, device=self.device)
    ...
    def generate_ma_noise(self, batch_size:int, length:int):
        ...
        '''
        seq = torch.randn(batch_size, self.noise_dim, self.ma_p, dtype=DATATYPE, device=self.device)
        noise = []
