...
def train_robustdqn(agent, env, writer, simulator_params, model_params, checkpoint_path=None):
    ...
    for episode in range(start_episode, n_episodes):
        ...
        cum_rewards = torch.zeros(batch_size, 1, device=device)
        obs, _ = env.reset()
        ...
        ep = episode+1
        with tqdm(total=steps, desc=f"Episode {ep}", mininterval=2) as step_bar:
            while not done.any():
...
def training_info(path):
    for file in os.listdir(path):
        if file.startswith('events'):
            filename = file
    new_path = path + filename
    event_acc = EventAccumulator(new_path, size_guidance={'scalars': 0})
    event_acc.Reload()
    ...
    for tag in tags:
        ...
        all_scalar_data[tag] = pd.DataFrame({'Q_Update_Step': steps, 'Value': values})
        
    return all_scalar_data